 ...
 *         Contains methods and fields for a GUI and displaying a restaurant ordering system app.
 */
...
public class ResturantViewModel extends JFrame
{
	...
	//ResturantView has a panel for menu items
	...
	//ResturantView has a middle panel.
	private JPanel middlePanel;
	////ResturantView has a bank balance label
	private JLabel bankBalanceLabel;
	//ResturantView has a add 20 dollars button
	private JButton add20DollarsButton;
	//ResturantView has a view recent orders button
	private JButton viewRecentOrder;
	//ResturantView has a second panel within itself to put to the top
	private JPanel middlePanel2;
	//ResturantView has a reorder button
	private JButton reOrder;
	//ResturantView has a refund recent order button
	private JButton refundRecentOrder;
	
	
	//All the stuff below is for the right panel
	//ResturantView has a right panel
	private JPanel rightPanel;
	//ResturantView has a cart label
	private JLabel cartTitle;
	//ResturantView has a label for the items
	private JLabel itemsLabel;
	//ResturantView has a button to remove item from cart
	private RemoveFromCart removeFromCartButton;
	//ResturantView has a panel for the items
	private JPanel cartItemsPanel; 
	//ResturantView has a subtotal label
	private JLabel subtotal;
	//ResturantView has a checkout button
	private JButton checkout;
	//ResturantView has a panel to put label for subtotal and checkout button
	private JPanel rightPanelEnd;
	//ResturantView has a border for its panels
	...
	public ResturantViewModel(Menu passedMenu, Customer passedUser)
	{
		...
		//set the layout to be a grid layout where it will stack to be the menu name and a button.
		menuItemsPanel.setLayout(new GridLayout(0,2));
		
		// a for loop that cycles through the menu txt file and adds them to the menuItemsPanel panel
		for(int i=0; i < menu.getNumberOfItemsInMenu(); i++)
		{
			...
			//creates a button for each item and passes in the item to the button. This will give a
		...
		//adds the panel to the actual middle panel and puts that panel to the top.
		...
		//set layout to be a grid layout where its name of item and button to remove it
		...
		//add the panel with checkout button and subtotal label to the right panel
		rightPanel.add(rightPanelEnd,BorderLayout.SOUTH);
		
		//set the layout of the frame to be a grid layout where there will be 3 sections 
	...
	/**
	 * Pass a item object with a quantity and display it to the user when the press add to cart button.
	 * @param item
	...
	public void removeItem(Item item) {
		...
		//for loop that checks for all items in the cart and creates a button and label for each item
